...
# Exact-Log-MAP golden vectors (x_e from the EXACT-mode fixed-point reference,
# scripts/fixedpoint_constituent_decoder_logmap.m). The x_a/z_a stimulus is
# byte-identical to the Max-Log-MAP CSV (same seed, frame-for-frame); only x_e
# differs. The DUT is built with EXACT_LOGMAP=true (see Makefile -g override).
VECTOR_PATH = (
    Path(__file__).resolve().parents[2]
    / "vectors"
    / "constituent_decoder_logmap.csv"
)

# Pinned extrinsic width (design.md "Fixed-point format - pinned"; matches the
# DUT generic default W_XE). Used only to read the streamed two's-complement
# x_e; the bit-exact compare is a plain integer compare.
W_XE = 18
...
def load_vectors():
    ...
    Schema (hdl/vectors/constituent_decoder_logmap.csv): K, x_a, z_a, x_e where
...
@cocotb.test()
async def matches_exact_logmap_reference_golden_vectors(dut):
    """Inner bit-exact gate: with EXACT_LOGMAP=true the HDL x_e must equal the
    exact-Log-MAP fixed-point oracle CSV (the max* Jacobian-corrected path)."""
    ...
    for K, x_a, z_a, x_e_exp in load_vectors():
        ...
        # ---- Collect N extrinsic codes streamed under out_valid; out_last marks
        # the final word. The core streams the backward sweep, emitting x_e(N-1)
        # first down to x_e(0) last (out_last on the index-0 word), i.e. the same
        # REVERSE index order as the P1 lane; we un-reverse below before compare.
        collected = []
        last_seen = False
        guard = 8 * N + 64
        ...
        # ---- Bit-exact compare against the exact-Log-MAP golden CSV.
    ...
    dut._log.info(f"all {n_frames} exact-Log-MAP frames bit-exact PASS")
