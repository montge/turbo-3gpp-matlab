"""Two-case GHDL self-check driver for the DE2 board demo.
...
use, and asserts the self-check verdict in each case:
  ...
  1. CORRUPT_IDX = -1  (default; the real board behaviour) -> must reach PASS
     for the genuine K=40 golden vector;
  2. CORRUPT_IDX = 5   (one expected output bit flipped)   -> must reach FAIL,
...
def run_case(corrupt_idx: int, expect: str) -> None:
    sim_args = f"-gCORRUPT_IDX={corrupt_idx} --vcd=tx_chain_de2_top.vcd"
    cmd = [
        "make",
        "SIM=ghdl",
...
def main() -> int:
    ...
    run_case(-1, "pass")
    print("== case 2: CORRUPT_IDX=5  -> expect FAIL ==")
    run_case(5, "fail")
