# fpga-tx-chain-de2-demo Specification

## Purpose
Defines the Terasic DE2 (Cyclone II `EP2C35F672C6`, Quartus II 13.0sp1) board
demonstration that runs the simulator-verified `tx_chain_top` core unmodified,
clocked by `CLOCK_50`, and self-checks its streamed length-`E` output on-chip
against a committed `tx_chain` golden vector for a fixed small `K` (no host
link) — covering the board wrapper, on-chip golden ROM, self-check FSM, 50 MHz
timing closure, artifact isolation, and the hardware-gated program-and-observe
pass criterion.
