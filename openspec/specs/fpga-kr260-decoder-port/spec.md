# fpga-kr260-decoder-port Specification

## Purpose
Defines the AMD Kria KR260 (Zynq UltraScale+ K26 SOM, `xck26-sfvc784-2LV`,
Vivado 2025.2.1) board demonstration that runs the simulator-verified iterative
`turbo_decoder_top` core unmodified at `K = 512` as a PL-only bitstream — clocked
by a minimal Zynq PS block design's `pl_clk0` at 100 MHz — and self-checks its
streamed `K` hard-decision bits on-chip against a committed channel-LLR golden
vector (no host link), reporting PASS/FAIL on two user LEDs and downloaded over
the on-board FT4232H JTAG. It covers the board wrapper, the PS clocking BD (with
a behavioural self-clocking sim stub), the on-chip golden ROM, the self-check
FSM, Vivado timing closure on the K26, the pinned toolchain/target, and the
hardware-gated program-and-observe pass criterion.
