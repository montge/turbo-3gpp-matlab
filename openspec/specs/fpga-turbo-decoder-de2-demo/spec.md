# fpga-turbo-decoder-de2-demo Specification

## Purpose
Defines the Terasic DE2 (Cyclone II `EP2C35F672C6`, Quartus II 13.0sp1) board
demonstration that runs the simulator-verified iterative `turbo_decoder_top`
core unmodified at `K = 512`, clocked by a PLL-derived ~12.5 MHz domain (the
α-recurrence caps Fmax below 50 MHz), and self-checks its streamed `K`
hard-decision bits on-chip against a committed channel-LLR golden vector (no host
link) — covering the board wrapper, PLL clock, on-chip golden ROM, self-check
FSM, PLL-domain timing closure, artifact isolation, and the hardware-gated
program-and-observe pass criterion.
