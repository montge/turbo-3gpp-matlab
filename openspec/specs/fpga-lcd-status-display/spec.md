# fpga-lcd-status-display Specification

## Purpose
Defines the shared HD44780 16×2 character-LCD controller (`hdl/boards/hd44780_lcd.vhdl`)
and its additive, human-readable status output on the Terasic DE2 board demos —
a fixed demo label plus a `RUNNING`/`PASS`/`FAIL` line driven from each demo's
existing self-check FSM (held a minimum ~1.5 s with an always-on heartbeat),
without modifying the verified cores, golden vectors, or LED/7-seg verdict path.
