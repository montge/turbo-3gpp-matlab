...
## Purpose
Defines the `plot_BLER_vs_SNR` and `plot_SNR_vs_A` Monte-Carlo simulation drivers, including the QPSK channel model, the HARQ retransmission loop over `rv_idx_sequence`, the early-stop criteria (`target_block_errors`, `target_BLER`), and the `results/` text-file format used to persist incremental measurements.
## Requirements
...
5. Persists incremental results to `results/BLER_vs_SNR_<A>_<R>_<retx>_<approx>_<target_errors>_<EsN0_start>_<seed>.txt`, where every numeric field is formatted with `num2str` and the floating-point coding rate `R` SHALL be formatted via `num2str(R)` to match the existing implementation (e.g. `1/3` produces `0.33333`). The file contains a header row and one row per simulated `EsN0`.
...
The system SHALL provide a `plot_SNR_vs_A` driver that performs the same encode/decode loop as `plot_BLER_vs_SNR` but sweeps the information-block length `A` for a fixed `target_BLER`, interpolating the `EsN0` that achieves the target BLER for each `A` and writing results to `results/SNR_vs_A_<R>_<retx>_<approx>_<target_BLER>_<seed>.txt` (every numeric field formatted with `num2str`).
