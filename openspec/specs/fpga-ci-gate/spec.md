# fpga-ci-gate Specification

## Purpose
Defines the CI gate that runs every cocotb/GHDL HDL lane against the committed
golden-vector CSVs on each pull request and push, and the single reusable lane
runner shared by CI and developers, so HDL regressions are gated rather than
