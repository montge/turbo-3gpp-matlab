...
- [ ] 1.1 Extract the **K=512** row of `hdl/vectors/turbo_decoder_top.csv` and
...
- [ ] 1.2 Add an on-chip golden-vector ROM `hdl/boards/de2/turbo_decoder_golden_pkg.vhdl`
...
- [ ] 1.3 Add an `altpll` megafunction (Cyclone II) deriving **~12.5 MHz** from
...
- [ ] 1.4 Add the self-check FSM (in the wrapper): `CH_RESET` → `CH_START`
...
- [ ] 1.5 Add the DE2 board wrapper `hdl/boards/de2/turbo_decoder_de2_top.vhdl`
...
- [ ] 1.6 Add the Quartus project + constraints under `hdl/boards/de2/`:
...
- [ ] 2.1 Add `hdl/sim/turbo_decoder_de2/` (Makefile + test module +
...
- [ ] 2.2 Assert the verdict: with `CORRUPT_IDX = -1` (real board behaviour) the
...
- [ ] 3.1 Compile the DE2 decoder-demo project under Quartus II 13.0sp1 — Full
  Compilation, 0 errors, no unsupported-family / unconstrained-device errors;
  the `altpll` elaborates for Cyclone II.
- [ ] 3.2 Record the fit (LE / M4K / DSP / memory bits). Expectation from the
...
- [ ] 3.3 Confirm TimeQuest closes setup and hold on the **PLL-derived ~12.5 MHz
...
- [ ] 3.4 Confirm `git status` after compile shows only intended sources (no
...
- [ ] 5.1 Add an `hdl/boards/de2/turbo_decoder_de2_README.md` (mirror the TX
...
- [ ] 5.2 Run `npx openspec validate add-fpga-turbo-decoder-de2-demo --strict`
  and `npx openspec validate --all --strict` — both pass, no regression.
- [ ] 5.3 Re-run the relevant GHDL lanes (`turbo_decoder_de2` self-check at
