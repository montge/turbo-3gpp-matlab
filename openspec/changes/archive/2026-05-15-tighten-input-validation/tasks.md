...
- [x] 1.1 List every root-level source `.m` helper that participates in specs or tests.
- [x] 1.2 Record current direct validation checks and error identifiers for each helper.
- [x] 1.3 Mark already-complete issue seed items: encoded segment `G` multiple check, `turbo_decoder` negative iteration check, and chain `stepImpl` length checks.
...
- [x] 2.3 Add deltas for any additional helper validation gaps discovered during the catalogue sweep.
...
- [x] 3.3 Implement additional missing helper-level checks discovered during the catalogue sweep.
...
- [x] 4.3 Add tests for each additional helper-level check.
