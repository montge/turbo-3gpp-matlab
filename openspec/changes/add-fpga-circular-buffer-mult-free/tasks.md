...
- [ ] 1.1 Re-confirm (standalone Quartus II 13.0sp1 A&S at full `KW_MAX=18528`,
...
- [ ] 2.1 At `S_QCALC` entry latch the two block constants with shifts/mux only
...
- [ ] 2.2 Add a parallel running accumulator `k0_acc` updated on every existing
  `S_QCALC` accumulate step (`k0_acc <= k0_acc + k0_inc;` alongside
  `q_acc`/`q_cnt`), sized off the existing `k0` range (`0 to 8·KW_MAX`), seeded 0.
- [ ] 2.3 Replace the line-298/299 products with pure adds: when `q_acc >= N_cb`,
  `k0 <= k0_acc + two_rtc; m_rem <= k0_acc + two_rtc;` (no `R_TC*…`, no `q*rv`).
- [ ] 2.4 Confirm the `q`/`pos`/`mod` recurrences, the parity decode
...
- [ ] 3.1 Re-run the `circular_buffer` cocotb/GHDL lane
...
- [ ] 3.2 Confirm `hdl/vectors/circular_buffer.csv` (and all other golden
  vectors) are **byte-identical** to master (`git diff master -- hdl/vectors/`
  empty); no vector regenerated.
- [ ] 3.3 Re-run the full suite `scripts/run_all_hdl_lanes.sh` — all lanes PASS,
  confirming no neighbouring core regresses.

## 4. Outer gate — Quartus fit at full `KW_MAX`, DSP = 0

- [ ] 4.1 Re-synthesize `circular_buffer` standalone at full `KW_MAX=18528`
...
- [ ] 4.2 Confirm the **M4K inference is retained** (`Total memory bits` ≈ 49,152
  / 12 M4K segments, unchanged) and logic elements are roughly flat or lower;
  record the LE / memory-bit / DSP counts before → after.
- [ ] 4.3 Confirm **`Fmax ≥ 50 MHz`** on the read clock (positive setup + hold
...
- [ ] 4.4 Confirm `git status` shows only the intended `hdl/rtl/circular_buffer.vhdl`
  edit (no `db/`, `output_files/`, `*.sof`, report artifacts; build in a scratch
  dir outside the repo).

## 5. Documentation + validation

- [ ] 5.1 Update the `circular_buffer.vhdl` header to record that the start-offset
...
- [ ] 5.2 Run `npx openspec validate add-fpga-circular-buffer-mult-free --strict`
