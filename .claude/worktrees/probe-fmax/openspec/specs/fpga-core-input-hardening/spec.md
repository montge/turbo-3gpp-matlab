# fpga-core-input-hardening Specification

## Purpose
Defines additive, out-of-contract input-validation and cleanliness guards for
the existing HDL cores (zero/over-range size parameters, unsupported `K`,
`qpp_rom.done` documentation, generator directory creation, doc code-fence
languages) such that every currently-verified golden vector remains bit-exact
and invalid inputs take a defined safe path instead of undefined behaviour.
