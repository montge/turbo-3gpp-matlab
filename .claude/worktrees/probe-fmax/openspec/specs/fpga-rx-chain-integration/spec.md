# fpga-rx-chain-integration Specification

## Purpose
Defines the integrated hardware LTE receive chain (`rx_chain_top` =
`de_rate_matching_top` → `turbo_decoder_top`): soft de-rate-matching that inverts
`rate_matching_top` on channel LLRs (inverse circular-buffer soft-combine + inverse
subblock-interleave, reconstructing the `3×(K+4)` W_EXT `d_a` load matrix) feeding
the unmodified iterative turbo decoder, verified bit-for-bit against the software
de-rate-match reference and closing the TX → BPSK+AWGN → RX BER loop, reusing all
sub-cores unmodified and fitting the Cyclone II `EP2C35F672C6`.
