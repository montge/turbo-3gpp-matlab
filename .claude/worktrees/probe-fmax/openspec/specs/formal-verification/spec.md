...
## Purpose
Defines the canonical contract for the formal-verification artifacts under `proofs/`. Covers three independent toolchains chosen for fit: Lean 4 (CRC matrix-vs-polynomial-division equivalence, QPP interleaver bijection across all 188 supported K, constituent encoder zero-state termination), TLA+ / TLC (HARQ protocol safety invariant and bounded liveness over the configured retransmission sequence), and Cryptol + SAW (independent bit-level CRC equivalence against a translated C reference, discharged by Z3). Also specifies the traceability matrix (`proofs/traceability.md`) mapping every proof obligation to the OpenSpec capability/scenario it discharges, and the CI `verify-*` jobs that re-check each proof with pinned toolchain versions on every pull request.
## Requirements
