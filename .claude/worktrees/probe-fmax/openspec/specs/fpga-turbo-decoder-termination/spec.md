# fpga-turbo-decoder-termination Specification

## Purpose
Matures the P2 iterative turbo-decode loop into a board-neutral fixed-point LTE
code-block decoder (the P3 increment of the staged decoder roadmap): the Octave
fixed-point reference extended with CRC-aided early termination (returning
`iterations_performed`), filler-bit handling (`NaN → +inf → MAX_SENT` reusing the
P1 ±inf sentinel), and HARQ soft accumulation across retransmissions; a
synthesizable VHDL `crc24_check` core (CRC24A/CRC24B) and the `turbo_decoder_term_top`
that folds early-stop control, filler mapping, and HARQ accumulate around the
reused P1/P2 cores unmodified; and its two-tier verification — an inner cocotb/GHDL
golden-vector lane bit-exact on decoded bits AND `iterations_performed`, and an
outer bounded BER / early-stop / CRC-pass-rate check against float
`turbo_decoder.m` / `turbo_decoding_chain.m`.
