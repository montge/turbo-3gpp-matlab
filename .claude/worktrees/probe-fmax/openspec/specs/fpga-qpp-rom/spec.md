# fpga-qpp-rom Specification

## Purpose
Defines the generated standardized QPP parameter ROM (TS36.212 Table 5.1.3-3),
the `K → (d0, step)` lookup with a supported flag, and the standalone hardware
turbo-encode datapath that integrates the verified QPP-interleaver and
turbo-encoder cores so the FPGA produces the `3 × (K+4)` encoded matrix from
just a code block and `K`, all verified against the software golden model.
